...
# Download all needed packages and check that every package in the list has a ".desc" file 
...
while read -r pkg_name pkg_source || [[ -n "$pkg_name" ]]; do
    ...
    if [[ ! -f "$desc_file" ]]; then
        log_error "Description file (.desc) not found for package: $pkg_name"
    fi
...
log_info "Deploying files using local staging..."
...
while read -r pkg_name pkg_source || [[ -n "$pkg_name" ]]; do
    ...
    iconv -f UTF-8 -t ISO-8859-1 "$desc_file" | while IFS=$'\t ' read -r src_raw dest_raw || [[ -n "$src_raw" ]]; do
        ...
        # if we are about copying a directory, remove its name from destination not to create "directory in directory"
        if [[ -e "$real_src" ]]; then
            if [[ -d "$real_src" ]] ; then
